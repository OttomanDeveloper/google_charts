...
abstract class BaseChart<D> {
  ...
  bool pointWithinRenderer(Point<double> chartPosition) {
    return _usingRenderers.any(
      (String rendererId) => getSeriesRenderer(
        rendererId,
  ...
  List<DatumDetails<D>> getNearestDatumDetailPerSeries(
    ...
    final boundsOverride = selectAcrossAllDrawAreaComponents
        ? drawableLayoutAreaBounds
        : null;

    final details = <DatumDetails<D>>[];
    for (final String rendererId in _usingRenderers) {
      details.addAll(
        getSeriesRenderer(rendererId).getNearestDatumDetailPerSeries(
          drawAreaPoint,
  ...
  void draw(List<Series<dynamic, D>> seriesList) {
    ...
    var processedSeriesList = List.of(
      seriesList.map<MutableSeries<D>>(makeSeries),
    );
  ...
  void configureSeries(List<MutableSeries<D>> seriesList) {
    ...
    for (final entry in rendererToSeriesList.entries) {
      getSeriesRenderer(entry.key).configureSeries(entry.value);
    }
  }
  ...
  Map<String, List<MutableSeries<D>>> preprocessSeries(
    ...
    for (final entry in rendererToSeriesList.entries) {
      getSeriesRenderer(entry.key).preprocessSeries(entry.value);
    }
  ...
  void onPostLayout(Map<String, List<MutableSeries<D>>> rendererToSeriesList) {
    ...
    for (final entry in rendererToSeriesList.entries) {
      getSeriesRenderer(entry.key).update(entry.value, animatingThisDraw);
    }
  ...
  void paint(ChartCanvas canvas) {
    ...
    for (final LayoutView view in _layoutManager.paintOrderedViews) {
  ...
  void fireOnDraw(List<MutableSeries<D>> seriesList) {
    for (final LifecycleListener<D> listener in _lifecycleListeners) {
...
typedef LifecycleSeriesListCallback<D> =
    void Function(List<MutableSeries<D>> seriesList);
